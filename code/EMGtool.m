...
%% User Input 1
inputEMG.foot = ["RF"];
inputEMG.days = ["P56"]; % this one can only be one!
...
inputEMG.out_path = "\\134.95.227.209\AG_Aswendt_Projects\Student_projects\12_Prisca_Kuhnhäuser_2021\Analysis_Opto\RN";
    ...
    for d_index = 1:length(days)
        ...
        for limb_index = 1:length(foot)
            ...
            for freq_channel = 1:length(Hz)
                cur_path = char(fullfile(inputEMG.in_path, strcat(inputGroupe(g_index),'_',foot(limb_index)),days(d_index)));
                mat = dir([cur_path, '\*_', num2str(Hz(freq_channel)) 'Hz.mat']);
