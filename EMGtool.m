...
%% User Input 1
inputEMG.foot = ["RF","LF"];
inputEMG.days = ["Baseline","P28","P41"]; % this one can only be one!
inputEMG.Groupe = ["Stim"]; % this one can only be one!
inputEMG.Hz = [0.1,0.5,1,2,5]; % set the measuring frequencies: it is important to set them from low to high
%inputEMG.in_path = "\\134.95.227.209\AG_Aswendt_Projects\Student_projects\12_Prisca_Kuhnhäuser_2021\Analysis_Opto\RN";
inputEMG.in_path = "X:\TVA_Pyramidenbahnen\V3\EMG";

inputEMG.out_path = "X:\TVA_Pyramidenbahnen\V3\EMG\evaluation";
...
ExperimentType = 'Chemogenetic';
Region = 'RN'; %we have more regions then 1
%% User Input 2
    ...
    for d_index = 1:length(days)
        ...
        for limb_index = 1:length(foot)
            ...
            for freq_channel = 1:length(Hz)
                cur_path = char(fullfile(inputEMG.in_path,days(d_index)));
                
                mat = dir(strcat(cur_path, '\*', foot(limb_index) ,'*-' ,num2str(Hz(freq_channel)) ,'Hz.mat'));
                %strcat(inputGroupe(g_index),'_',
                for animal_number = 1:length(mat)

                    inputEMG.in_path_re = [cur_path, '\', mat(animal_number).name];
            ...
            for animal_number = 1:length(mat)
                for reps = 1:S(3)
                    if isempty(mat)
                        ...
                        Name_temp = split(mat(animal_number).name,'_');
                        %Name_temp = join(Name_temp(1:end-1),"_");
                        Names{O} = [Name_temp];
